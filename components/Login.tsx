  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (role === 'admin') {
      if (password === 'DejSeg@22') {
        onLogin({ role: 'admin', name: t('keeperRole') });
      } else {
        setError('Incorrect administrator password.');
      }
    } else {
      onLogin({ role: 'user', name: name.trim() || t('explorerRole') });
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-4 bg-gradient-to-br from-rose-100 to-amber-100 relative overflow-hidden h-full w-full select-none">
      ...
      <div className="w-full max-w-md bg-white/70 backdrop-blur-xl p-8 rounded-[3rem] shadow-2xl border-4 border-white transform transition-all">
        <h2 className="text-4xl font-kids text-rose-600 text-center mb-8 drop-shadow-sm">{t('whoIsVisiting')}</h2>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          {role === 'user' ? (
            <div className="space-y-2">
              <label className="text-[10px] font-black text-rose-400 uppercase px-2 tracking-widest">{t('yourName')}</label>
              <input 
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder={`${t('explorerRole')}...`}
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-rose-400 uppercase px-2 tracking-widest">{t('secretWord')}</label>
              <input 
                type="password"
                value={password}
